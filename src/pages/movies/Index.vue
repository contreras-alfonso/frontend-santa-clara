<template>
  <q-page class="q-pa-md">
    <div class="row items-center justify-center">
      <div class="col-md-11 col-12 q-gutter-y-md">
        ...
        <TableSkeleton v-if="loading.page" :rows="6" :columns="6" />
        ...
        >
          ...
          <template v-slot:body="props">
            <q-tr :props="props">
              <template v-for="col in props.cols" :key="col.name">
                <q-td
                  v-if="col.name !== 'status' && col.name !== 'image'"
                  :key="col.name"
                  :props="props"
                >
                  {{ col.value }}
                ...
                <q-td v-if="col.name == 'image'" :key="col.name" :props="props">
                  <q-img
                    width="60px"
                    style="height: 60px"
                    spinner-color="grey"
                    ...
                    loading="lazy"
                    fit="contain"
                    :src="col.value ? col.value : '/images/no-image.jpg'"
                  />
                </q-td>
              </template>
  ...
  <ConfirmDelete
    ...
    :msg="`la ${singularTitle}`"
    @on-delete="onDeleteMovie"
...
const visible: string[] = ['id', 'image', 'name', 'publication_date', 'status'];
...
const onHandleUpdateStatus = async (id: number) => {
  if (id) {
    onSpinner(true);
    try {
      await movieStore.updateStatus(id);
...
const onDeleteMovie = async () => {
  if (dialogs.value.delete.entityId) {
    onSpinner(true);
    try {
      await movieStore.delete(dialogs.value.delete.entityId);
