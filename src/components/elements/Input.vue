<template>
    <q-input :label="label" :outlined="outlined" v-model="inputVal" @update:model-value="onInput" color="grey-10"
        :label-color="labelColor ? labelColor : 'grey-7'" :bg-color="bgColor ? bgColor : 'white'" :input-class="[
        ...
        ]" :readonly="localReadOnly" :maxlength="maxLength ? maxLength : undefined" :rounded="isRounded ? true : false"
        :square="isSquare ? true : false"
        :type="localIsPwd ? 'password' : isMail ? 'email' : isTextArea ? 'textarea' : 'text'" :rules="computedRules"
        :debounce="time" hide-bottom-space :prefix="prefix" no-error-icon :loading="localIsLoading"
        ...
        <template v-slot:append>
            <q-icon v-if="isColor" name="colorize" :color="iconColor ? iconColor : 'primary'" class="cursor-pointer">
                <q-popup-proxy cover transition-show="scale" transition-hide="scale">
                    <q-color v-model="colorValue" />
                </q-popup-proxy>
            </q-icon>
            <q-icon v-if="isPwd" :name="localIsPwd ? 'visibility_off' : 'visibility'" class="cursor-pointer"
                :color="iconColor ? iconColor : 'grey'" @click="localIsPwd = !localIsPwd" />
        </template>
